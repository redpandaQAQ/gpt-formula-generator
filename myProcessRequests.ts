 ...
 */
export async function myProcessRequests(interactivePrompt: string, inputFileName: string | undefined, processRequest: (request: string) => Promise<void>) {
    if (inputFileName) {
        const lines = fs.readFileSync(inputFileName).toString();
        if (lines.length) {
            console.log(interactivePrompt + lines);
            await processRequest(lines);
        }
    }
    else {
